...
"use strict"
let projectDatatablesDataSourceAjaxServer = function () {

    let initTable1 = function () {
        let fetch_project_datatable = $('#fetch_project_datatable');
        ...
        // begin first table
        fetch_project_datatable.DataTable({
            "fixedHeader": {
                "header":true
            },
            responsive: true,
          ...
          <'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7 dataTables_pager'lp>>`,
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        return ` <span style="width: 120px;"><span class="label label-${getStatusLabelColor(row?.project_status?.code)} label-dot mr-2"></span>
                                    <span class="font-weight-bold text-${getStatusLabelColor(row?.project_status?.code)}">${row?.project_status?.name}</span>
                                </span>`
                    },
                    orderable: false,
                    searchable: false,
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        let delete_url = route.admin.projects.destroy.replace("{project_uuid}", row?.uuid);
                        let show_url = route.admin.projects.show.replace("{project_uuid}", row?.uuid);
                        return `
                            <a href="${show_url}" class="btn btn-icon btn-light btn-sm">
                            ...
                            <a href="#" class="btn btn-icon btn-light btn-sm delete_project_btn" data-value="${row?.uuid}">
...
$(document).ready(function () {
    projectDatatablesDataSourceAjaxServer.init();
});
